...
class LLMClient(Protocol):
    ...
    async def chat(self, messages: Sequence[ChatMessage], *, temperature: float, max_tokens: int) -> Result[str, LLMError]: ...
...
### Chat module — applied patterns

The `ss chat` feature reuses the same playbook as the rest of the codebase:

- **Template Method** — `BaseLLMClient.chat()` validates inputs/result-types and dispatches to provider-specific `_chat_model`.
- **Repository** — `ChatRepository` port + `SqliteChatRepository` adapter; the chat use-case never imports SQLite.
- **Facade** — `ChatService` exposes a thin façade over the repository, LLM, portfolio, and universe; the CLI talks only to this façade.
- **Result/Either** — `chat()` and `ChatService.send()` return `Result[str, LLMError]` so the CLI never has to wrap LLM I/O in try/except.
- **Strategy** — context-injection logic (portfolio block, known-ticker block) is composed of pure functions; swap or extend without touching the CLI.
